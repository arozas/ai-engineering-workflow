...
try {
    ...
    foreach ($requiredPath in @($projectPath, $projectSchemaPath, $gateSchemaPath, $runnerPath)) {
    ...
    $requestedIds = @(
        $ModuleId |
            ForEach-Object { @(([string]$_).Split(',', [StringSplitOptions]::RemoveEmptyEntries)) } |
    ...
    if ($requestedIds.Count -gt 0) {
        ...
        if ($missingIds.Count -gt 0) { throw "Unknown quality-gate module(s): $($missingIds -join ', ')" }
        $selectedModules = @($availableModules | Where-Object { $requestedIds -contains [string]$_.id })
    }
    else { $selectedModules = $availableModules }
    if ($selectedModules.Count -eq 0) { throw 'At least one module must be selected for quality gates.' }
    ...
    foreach ($module in $selectedModules) {
        ...
        if (-not $modulePath.StartsWith($repositoryRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
        ...
        $configuredCount = 0
        foreach ($phaseName in $phaseOrder) { $configuredCount += @($module.quality.$phaseName).Count }
        $moduleFailed = $false
        $stopModule = $false
        $phaseResults = @()
