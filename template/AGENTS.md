...
For an existing run, call `workflow_next` once and perform one returned legal action. When that typed tool is absent or explicitly unavailable, its only fallback is exactly `pwsh -NoProfile -File .ai/scripts/workflow-state.ps1 -Action Show -RunId <run-id> -Transport deterministic-script`, once. Never substitute `Validate`, search for the tool, or retry. Do not repeat unchanged state, profile, gate, classifier, review, delegation, or search operations. Stop at the next approval, blocker, escalation, or terminal state.
...
The script mapping is: `workflow_state` -> `.ai/scripts/workflow-state.ps1` with the requested action and `-Transport deterministic-script`; `workflow_next` -> `.ai/scripts/workflow-state.ps1 -Action Show -RunId <run-id> -Transport deterministic-script`; `workflow_gate` -> `run-quality-gates.ps1`; `workflow_dotnet_solution_add` -> `add-dotnet-project.ps1`; `workflow_fast_path` -> `fast-path-check.ps1`; `workflow_validate_project` -> `validate-project.ps1`; `workflow_profile_project` -> `profile-project.ps1`; `workflow_bootstrap_prepare` -> `prepare-bootstrap-proposal.ps1`; `workflow_bootstrap_apply` -> `apply-bootstrap-proposal.ps1`; `workflow_validate_diagnosis` -> `validate-diagnosis.ps1`; `workflow_delivery_check` -> `delivery-check.ps1`; and reviewer tools -> `record-review.ps1` with the exact role and `-Transport deterministic-script`.
...
5. Write a schema-version-2 `verification.json` execution contract with exact affected modules, expected paths, dependency versions, decisions, constraints, estimates, and commands. Resolve every material decision, wait for explicit human approval of both artifacts, then record `PLAN_APPROVED` before modifying production code.
...
LLM judgment never overrides command results. Quality gates must run through `workflow_gate` or its exact deterministic script fallback for an approved run; agents cannot select a smaller module set and must never create or edit `.ai/runtime/<run-id>/gates.json` manually. Planning must persist a schema-version-2 `.ai/runtime/<run-id>/verification.json` execution contract with exact affected modules, expected paths, dependency versions and reasons, settled decisions, constraints, estimates, and commands; schema version 1 is legacy compatibility only. Mandatory task-specific commands absent from `.ai/project.json` must be explicitly approved there, copied and hashed with the plan, and merged into the frozen matrix. Before commands run, changed paths must match the frozen scope and generated paths must be ignored and untracked. After commands, generated output must not contain the workflow control plane. A gate passes only when every command in that approved matrix exits successfully, in order, and the worktree remains unchanged during verification. Missing, skipped, or unexecuted required commands must be reported as NOT RUN, never PASS. Repository hygiene produces `GATE_BLOCKED`; scope drift produces `PLAN_INVALIDATED`; neither is a normal command failure. `workflow_state` or its exact fallback independently validates the run ID, exact module and command matrix, execution contract, schema, project and runner hashes, derived verdict, worktree fingerprint, repository hygiene, plan scope, generated control-plane output, and control-plane fingerprint.
