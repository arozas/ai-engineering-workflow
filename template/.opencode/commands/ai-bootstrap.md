...
1. Run `workflow_bootstrap_prepare`.
2. If the typed tool is unavailable, run the exact fallback command `pwsh -NoProfile -File .ai/scripts/prepare-bootstrap-proposal.ps1`.
3. If both are unavailable or fail before returning JSON, stop with `BOOTSTRAP BLOCKED: PROFILE TOOL UNAVAILABLE`.
4. If the result is `BOOTSTRAP NOT APPLICABLE` or `NO PROJECT MODULES DETECTED`, repeat that verdict and stop.
5. If the result is `BOOTSTRAP_PROPOSAL_READY`, summarize only the returned JSON: proposal root, fingerprint, modules, files, risks, and next step.
