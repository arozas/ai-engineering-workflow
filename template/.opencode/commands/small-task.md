...
If eligible, persist the compact micro-plan and wait for explicit approval. Record approval before implementation. After approval, implement the smallest change, run and persist the exact verification and applicable gates, revalidate actual scope from Git, and persist a compact independent `quick-reviewer` verdict. Use no more than one correction cycle.
