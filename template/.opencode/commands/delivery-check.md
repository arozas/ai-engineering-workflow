...
Load `workflow-state` and `delivery-safety`. Require a validated persisted run in `READY_FOR_DELIVERY`, then delegate a read-only delivery inspection to `delivery` using `.ai/scripts/delivery-check.ps1`.
