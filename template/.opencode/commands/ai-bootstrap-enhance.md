...
1. Run `workflow_bootstrap_prepare` once without force. Continue only for `BOOTSTRAP_PROPOSAL_READY` or `BOOTSTRAP_PROPOSAL_CURRENT`.
...
3. After it returns, run `workflow_bootstrap_apply` once with `dryRun: true`.
