...
Every expected file must have a reason. Plans must be specific enough that the developer does not need to invent architecture. Materialize a schema-version-2 execution contract in `.ai/runtime/<run-id>/verification.json` using `.ai/task-verification.schema.json`. Record the exact affected modules, expected file actions, exact dependency versions and their evidence source, closed decisions, constraints, estimated files/lines, and task-specific commands. `unresolvedDecisions` must be empty before requesting approval. An empty command array is required when configured gates are sufficient. Every manifest command must appear in the plan and every mandatory plan command absent from `.ai/project.json` must appear in the manifest. For .NET, reuse the configured explicit `.sln`, `.slnx`, or project target in task-specific commands; never propose a bare `dotnet restore`, `dotnet build`, or `dotnet test` when the module can contain multiple build files. Schema version 1 remains valid only for already persisted runs created by earlier workflow releases. End with `WAITING FOR APPROVAL`; no production edit is allowed before explicit approval of both artifacts.
