...
2. Write the complete proposed plan to `.ai/runtime/<run-id>/plan.md` and a schema-version-2 `.ai/runtime/<run-id>/verification.json` execution contract. Freeze the exact affected modules, expected paths, dependency versions and reasons, decisions, constraints, estimates, and mandatory run-specific commands. `unresolvedDecisions` must be empty. Schema version 1 is compatibility for already persisted runs, not a shortcut for new planning.
...
5. Run `workflow_gate` with the run ID. It first enforces the approved path scope and repository hygiene, executes the frozen matrix only when safe, and rejects control-plane files in generated output. It writes factual evidence to `.ai/runtime/<run-id>/gates.json`; then call `RecordGates` with `PASS`, `FAIL`, or `BLOCKED`. `GATE_BLOCKED` requires repository-policy remediation outside the correction loop. `PLAN_INVALIDATED` requires a revised and explicitly approved plan.
